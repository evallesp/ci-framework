...
* `cifmw_reproducer_controller0_user`: (String) User on controller-0. Defaults to the `ansible_user_id` associated with controller-0 if available, otherwise defaults to `zuul`.
...
* `cifmw_reproducer_controller0_basedir`: (String) Path to the `ci-framework-data` dir on controller-0. Defaults to `"{{ cifmw_reproducer_controller0_user_dir }}/ci-framework-data"`
